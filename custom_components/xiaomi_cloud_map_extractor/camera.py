...
ROOMS_CLEANING_SCHEMA = vol.Schema({
    vol.Required(CONF_ENTITY_ID): cv.entity_domain(vacuum.DOMAIN,),  # Validación como entity_id de Home Assistant
    vol.Required("rooms_id"): cv.string,  # Convertir a entero si es posible
})
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    async def handle_rooms_cleaning(call):
        ...
        entity_id = call.data.get("entity_id")
            ...
            # Verificar que entity_id fue proporcionado
        if not entity_id:
            _LOGGER.error("No se proporcionó entity_id para iniciar la limpieza.")
            return

        # Definir los siid y aiid específicos para la acción de limpieza en tu dispositivo Xiaomi
        siid = 7  # Service ID (debes ajustar este valor a tu dispositivo)
        aiid = 3  # Action ID para iniciar limpieza (debes ajustarlo también)

        # Parámetros adicionales (como `rooms_id` si es necesario)
        params = []
        if rooms_id:
            params.append(str(rooms_id))  # Agrega rooms_id si el dispositivo lo soporta
            params.append(0)
            params.append(1)

        # Llamar al servicio de xiaomi_miot para ejecutar la acción
        await hass.services.async_call(
            "xiaomi_miot",  # Dominio del servicio
            "call_action",  # Acción definida en `call_action`
            {
                "entity_id": entity_id,  # La aspiradora Xiaomi que vas a controlar
                "siid": siid,            # El Service ID específico para tu dispositivo
                "aiid": aiid,            # El Action ID para iniciar la limpieza
                "params": params,        # Parámetros opcionales como `rooms_id`
            }
        )
